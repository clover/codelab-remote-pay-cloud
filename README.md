...
The POS has a **Connect** button that has a bound `onclick` handler to invoke a `connect()` function defined in `index.js`. The complete `connect()` function will pair the POS and the Clover device.
